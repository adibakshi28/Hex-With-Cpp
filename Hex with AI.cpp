...
'E' -> Empty Node
'X' -> Player 1 occupied Node (Computer/ Human)
'O' -> Player 2 occupied Node (Computer/ Human)
...
The AI for the game is implemented using Monte Carlo algorithm 
*/
...
int MoveAI(bool player1Move) {   // Returns optimum move for the current game ie moveCellIndex
	vector<char> orignalNodes = nodes;
	...
	int curMaxWin = 0;
	int curMaxWinMoveIndex = -1;

	for (int i = 0;i < emptyCells.size();i++) {
		cout << "#";
		int curMoveCellIndex = emptyCells[i];

		int runTimes = 200;
		int win = 0;

		for (int itr = 0;itr < runTimes;itr++) {   // Monte Carlo simulation run for runTimes times for curMoveCellIndex as its first move
			vector<int> emptyCellsCopy = emptyCells;
			random_shuffle(emptyCellsCopy.begin(), emptyCellsCopy.end());

			bool xMove = player1Move;

			if (xMove) {
				nodes[curMoveCellIndex] = 'X';
			}
			else {
				nodes[curMoveCellIndex] = 'O';
			}
			xMove = !xMove;

			for (int j = 0;j < emptyCellsCopy.size();j++) {
				if (emptyCellsCopy[j] == curMoveCellIndex) {
					continue;
				}
				if (xMove) {
					nodes[emptyCellsCopy[j]] = 'X';
				}
				else {
					nodes[emptyCellsCopy[j]] = 'O';
				}
				xMove = !xMove;
			}
		...
		if (curMaxWin < win) {
			curMaxWin = win;
			curMaxWinMoveIndex = curMoveCellIndex;
		}
	}
	...
	return curMaxWinMoveIndex;
}


int main() {  // Main Function
	do {
		cout << "Enter Board Size (min 6) : ";
		cin >> boardSize;
		cout << endl;
	} while (boardSize < 6);
	...
	do {
		...
		cin >> humanMoveFirst;
		cout << endl;
	} while (humanMoveFirst != 'y' && humanMoveFirst != 'n');
	...
	while (true) {
		...
		if (player1Move) {
			curMoveChar = 'X';
			if (humanMove) {
				cout << "Human Player (X) move ... Enter cell index : ";
				cin >> move;
			}
			else {
				cout << "Computer (X) move ...";
				moveCellIndex = MoveAI(player1Move);
			}
		}
		else {
			curMoveChar = 'O';
			if (humanMove) {
				cout << "Human Player (O) move ... Enter cell index : ";
				cin >> move;
			}
			else {
		...
		if (humanMove && (!IsValidMove(move, moveCellIndex) || nodes[moveCellIndex] != 'E')) {
			cout << endl;
	...
	if (player1Move) {
		if (humanMove) {
			cout << "Human Player (X) has won !!!...";
		}
		else {
			cout << "Computer (X) has won !!!...";
		}

	}
	else {
		if (humanMove) {
			cout << "Human Player (O) has won !!!...";
		}
		else {
			cout << "Computer (O) has won !!!...";
		}
	}
	cout << endl;
